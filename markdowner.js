...
graphql(createSchema(token), query).then(response => {
  ...
  const stats = project.epics.reduce(
    (stats, epic) => {
...
<p>This project has <strong>${stats.sum}</strong> ${displayUnit(
    stats.sum
  ).toLowerCase()} in <strong>${
    stats.count
  }</strong> stories contained in <strong>${stats.epics}</strong> epics</p>
...
${
  epic.stories.length > 0
    ? `<ol>${epic.stories.map(convertStoryToHtml(memberIndex)).join('')}</ol>`
    : ''
}
`
}

const convertCommentToHtml = memberIndex => comment =>
  comment.text ? `<p>@${memberIndex[comment.author]} - ${comment.text}</p>` : ''
...
<li>${markedNl2br(
  typeToEmoji(story.type) +
    ' ' +
    `${story.title} [${story.id}](${story.url}) ${
      displayPoints && story.points
        ? `- ${story.points} ${displayUnit(story.points)}`
        : ''
    }`
)}
${story.description ? `${markedNl2br(story.description)}` : ''}
${story.comments ? convertCommentsToHtml(memberIndex, story.comments) : ''}`
